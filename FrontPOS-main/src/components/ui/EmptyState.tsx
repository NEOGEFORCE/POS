    ...
    return (
        <Card className={`w-full border-2 border-dashed border-gray-200 dark:border-white/10 bg-transparent shadow-none ${compact ? 'py-4' : 'py-12'}`}>
            <CardBody className={`flex flex-col items-center justify-center text-center ${compact ? 'space-y-2' : 'space-y-4'}`}>
                <div className={`${compact ? 'p-2' : 'p-4'} bg-gray-50 rounded-2xl dark:bg-[#18181b]`}>
                    {icon || defaultIcon}
                </div>
                <div className={compact ? 'space-y-0' : 'space-y-1'}>
                    <h3 className={`${compact ? 'text-sm' : 'text-lg'} font-bold text-gray-900 dark:text-white uppercase tracking-tight tracking-tight`}>{title}</h3>
                    <p className={`${compact ? 'text-[10px]' : 'text-sm'} text-gray-500 dark:text-gray-400 max-w-xs uppercase font-medium tracking-tight opacity-60`}>
                        {description}
                    </p>
                </div>
