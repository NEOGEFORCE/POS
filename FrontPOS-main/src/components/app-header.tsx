...
export function AppHeader() {
  ...
  return (
    <header className="sticky top-0 z-[100] flex h-12 items-center justify-between border-b border-divider dark:border-white/10 bg-white dark:bg-zinc-900 px-3 shadow-md transition-colors">
      ...
      <div className="flex-1 hidden sm:flex flex-col items-center justify-center overflow-hidden px-1 sm:px-2">
        <div className="flex items-center gap-1 md:gap-3 bg-default-100/50 dark:bg-zinc-800/80 px-1 sm:px-4 py-1 rounded-lg md:rounded-xl border border-divider dark:border-white/5 backdrop-blur-md">
          <div className="flex flex-col items-center leading-none">
            ...
            <span className="text-[9px] md:text-sm font-black text-foreground tabular-nums tracking-tighter">{timeStr}</span>
          </div>
          <div className="w-px h-3 md:h-5 bg-divider dark:bg-white/10" />
          <div className="flex flex-col items-start leading-none pr-1">
            <span className="text-[6px] md:text-[8px] font-black text-default-500 dark:text-zinc-400 uppercase tracking-[0.2em] md:tracking-[0.3em] mb-0.5">Fecha</span>
            <span className="text-[9px] md:text-[10px] font-black text-default-700 dark:text-zinc-200 tracking-tighter truncate max-w-[80px] md:max-w-[150px]">{dateStr}</span>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-2 sm:gap-6">
        ...
        <div className="hidden lg:flex flex-col items-end">
          <p className="text-[10px] font-black text-foreground uppercase tracking-tighter leading-none mb-0.5">
            {user?.name || 'Operador'}
          </p>
