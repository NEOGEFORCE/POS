    ...
    <div className="flex items-center justify-between px-1">
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          <h1 className="text-[13px] font-medium uppercase tracking-tighter leading-none tracking-tight">GESTION DE <span className="text-zinc-900 dark:text-zinc-100">PERSONAL</span></h1>
...
export default function UsersPage() {
  ...
  const loadUsers = useCallback(() => {
    ...
    fetchUsers(token)
      ...
      .catch(() => toast({ variant: "destructive", title: "ERROR", description: "SINCRONIZACION DE PERSONAL FALLIDA" }))
  ...
  // SINCRONIZACION ZERO-F5: Usuarios en tiempo real
  useEffect(() => {
  ...
  const handleAddUser = useCallback(async (data: Partial<User>) => {
    ...
    if (existingLocal) {
      toast({ title: 'DOCUMENTO YA REGISTRADO', description: 'ABRIENDO PERFIL EXISTENTE PARA EDICION...' });
    ...
    try {
      ...
      toast({ variant: 'success', title: 'EXITO', description: 'ACCESO CREADO CORRECTAMENTE' });
      setAddDialogOpen(false);
      loadUsers();
  ...
  const handleEditUser = useCallback(async (data: Partial<User>) => {
    ...
    try {
      ...
      toast({ variant: 'success', title: 'EXITO', description: 'PERMISOS ACTUALIZADOS' });
  ...
  const handleDeleteUser = useCallback(async () => {
    ...
    try {
      ...
      toast({ variant: 'success', title: 'EXITO', description: 'ACCESO REVOCADO' });
  ...
  const handleLookupUser = useCallback(async (dni: string) => {
    ...
    if (existingUser) {
      toast({
        ...
        description: `EL DNI ${dni} YA EXISTE. ABRIENDO MODO EDICION PARA ${existingUser.name}.`
      });
