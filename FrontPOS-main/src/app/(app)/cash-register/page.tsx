      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-4 md:gap-6 flex-1">
        ...
        <div className="bg-[#18181b] border border-zinc-200 dark:border-white/5 rounded-[2rem] p-6 md:p-8 flex flex-col relative overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
          ...
          <div className="flex items-center gap-3 mb-8">
            <div className="h-8 w-8 rounded-2xl bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center">
              <Receipt size={14} className="text-gray-500 dark:text-zinc-500 dark:text-zinc-400" />
            </div>
            <h2 className="text-sm font-medium uppercase tracking-[0.2em] text-gray-500 dark:text-zinc-500 dark:text-zinc-400">Resumen del Turno</h2>
          </div>

          <div className="space-y-6 flex-1">
            <div className="flex flex-col">
              <span className="text-[11px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mb-1">1. Efectivo Entrante</span>
              <span className="text-2xl font-medium tracking-tight tracking-tighter text-gray-600 dark:text-zinc-300 tabular-nums">+{formatCurrency(closure?.totalCash || 0)}</span>
            </div>

            <div className="flex flex-col">
              <span className="text-[11px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mb-1">2. Efectivo Saliente</span>
        ...
        <div className="bg-[#18181b] border border-zinc-200 dark:border-white/5 rounded-[2rem] p-6 md:p-8 flex flex-col gap-6 md:gap-8 shadow-[0_8px_30px_rgb(0,0,0,0.12)] relative">
          ...
          <div className="flex flex-col gap-2 relative z-10">
            <label className="text-[12px] md:text-sm font-medium uppercase tracking-[0.3em] text-gray-500 dark:text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              <div className="flex flex-col gap-1">
                 <label className="text-[10px] font-medium uppercase tracking-widest text-gray-500 dark:text-zinc-500">Nequi Real</label>
                 <Input 
                   type="number"
                   placeholder="0"
              ...
              <div className="flex flex-col gap-1">
                 <label className="text-[10px] font-medium uppercase tracking-widest text-gray-500 dark:text-zinc-500">Daviplata Real</label>
                 <Input 
                   type="number"
                   placeholder="0"
            ...
            <div className="bg-rose-500/5 border border-rose-500/20 rounded-2xl p-4 mt-2 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <Input 
                  ...
                  classNames={{ inputWrapper: "bg-white dark:bg-zinc-950 border-rose-500/20", input: "font-bold text-gray-500 dark:text-zinc-500 dark:text-zinc-400 text-xs" }}
                />
              </div>
              <p className="text-[9px] text-gray-500 dark:text-zinc-500 tracking-tight">Este monto se registrara como un gasto del turno actual y se restara del saldo esperado.</p>
            </div>
          </div>

          {/* Calculadora Rapida */}
          <div className="flex flex-col gap-3 relative z-10">
            <label className="text-[10px] font-medium uppercase tracking-[0.2em] text-gray-500 dark:text-zinc-500">Calculadora Rapida (Opcional)</label>
              ...
              {['100000', '50000', '20000', '10000', '5000', '2000', '1000'].map(bill => (
                <Input
                  ...
                  classNames={{
                    ...
                    input: "font-medium text-right text-sm text-gray-600 dark:text-zinc-300"
                  }}
                  startContent={<span className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase">${bill.replace('000', 'K')}</span>}
                />
              ))}
              <Input
                type="number"
                ...
                startContent={<span className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 uppercase">Monedas $</span>}
              />
            </div>
          </div>
          ...
          <div className="mt-auto flex flex-col gap-4 relative z-10">
            {status === 'PENDING' && (
              <div className="h-24 md:h-32 border-2 border-dashed border-zinc-200 dark:border-white/10 rounded-2xl flex items-center justify-center text-center p-4">
                <span className="text-gray-500 dark:text-zinc-500 font-medium uppercase tracking-widest text-xs">Cuenta el dinero fisico e ingresalo arriba</span>
              </div>
            )}

            {status === 'BALANCED' && (
              <div className="h-24 md:h-32 bg-black/5 dark:bg-white/5 border border-emerald-500/30 rounded-2xl flex items-center justify-center p-6 animate-in fade-in zoom-in-95">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl md:text-4xl font-medium text-gray-600 dark:text-zinc-300 uppercase tracking-tight tracking-tighter">Caja Cuadrada</h2>
            ...
            <Button
              ...
              className={`w-full h-20 rounded-2xl font-medium text-xl md:text-2xl uppercase tracking-[0.2em] tracking-tight shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all duration-300 mt-2 ${status === 'PENDING'
                  ? 'bg-zinc-100 dark:bg-zinc-800 text-gray-500 dark:text-zinc-500 opacity-50'
