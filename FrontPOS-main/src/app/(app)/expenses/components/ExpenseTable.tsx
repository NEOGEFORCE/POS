            ...
            <TableBody
              emptyContent={
                <div className="py-24 text-center">
                  <span className="text-xs font-medium uppercase tracking-[0.4em] tracking-tight text-zinc-500 dark:text-zinc-400">Sin registros de egreso</span>
                </div>
              }
            >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col flex-1 min-h-0 min-w-0 h-full w-full">
                        ...
                        <span className="text-[8px] text-zinc-500 dark:text-zinc-400 font-bold tracking-widest mt-1 uppercase flex items-center gap-2">
                          {expense.category}
                  ...
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center gap-1">
                      <Chip
                        size="sm"
                        variant="flat"
                        className={`text-[9px] font-medium h-6 uppercase tracking-tight border-none px-2 ${expense.paymentSource === 'NEQUI' ? 'bg-pink-500/10 text-pink-500' :
                          expense.paymentSource === 'DAVIPLATA' ? 'bg-rose-500/10 text-rose-500' :
                            (expense.paymentSource === 'PRESTAMO' || expense.paymentSource === 'PREST.') ? 'bg-amber-500/10 text-amber-500' :
                              'bg-white/5 text-zinc-900 dark:text-zinc-100'
                          }`}
                      >
                      ...
                      {isAdmin && (
                        <div className="flex gap-1">
                          <Tooltip content="EDITAR" delay={0} closeDelay={0} showArrow classNames={{ content: "font-medium text-[9px] uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-white/5 text-white py-1 px-2 rounded-none shadow-[0_8px_30px_rgb(0,0,0,0.12)]" }}>
                            <button
                              className="h-8 w-8 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-white/5 text-zinc-900 dark:text-zinc-100 rounded-2xl hover:bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-white/5 hover:text-white border border-emerald-500/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all flex items-center justify-center active:scale-90"
            ...
            <div key={expense.id} className="card-base border-none p-4 rounded-2xl border border-gray-200 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col gap-3 shrink-0 transition-transform active:scale-[0.98]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <h3 className="text-[11px] font-medium text-zinc-900 dark:text-zinc-50 uppercase truncate max-w-[150px] tracking-tight leading-tight">{expense.description}</h3>
                    <span className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">{expense.category}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end leading-tight text-right">
                  <span className="text-[13px] font-medium text-rose-500 tabular-nums tracking-tight leading-none">${Number(expense.amount).toLocaleString()}</span>
                  <div className="flex flex-col items-end mt-1 gap-0.5">
                    <span className={`text-[7px] font-medium uppercase tracking-tighter leading-none ${expense.status === 'PENDING' ? 'text-amber-500' : 'text-gray-400'}`}>
                      {expense.status === 'PENDING' ? 'DEUDA PENDIENTE' : expense.paymentSource}
                    </span>
