                    ...
                    <>
                        <ModalHeader className="flex items-center gap-4 p-8 border-b border-gray-100 dark:border-white/5 rounded-t-[2rem]">
                            ...
                            <div className="flex flex-col min-w-0">
                                <span className="text-xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter leading-none">Venta Maestro #{sale.id}</span>
                            ...
                            <div className="ml-auto">
                                <Chip variant="dot" color="success" size="sm" className="font-bold border-gray-200 dark:border-white/10 uppercase tracking-widest text-[8px]">Sincronizado</Chip>
                            </div>
                        </ModalHeader>
                        <ModalBody className="p-8 gap-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    ...
                                    <div className="p-6 bg-gray-50 dark:bg-zinc-900 rounded-[2rem] border border-gray-200 dark:border-white/5 space-y-4">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[9px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-[0.2em]">Nombre</span>
                                            <span className="text-sm font-black text-gray-900 dark:text-white uppercase italic">{sale.client?.name || 'Consumidor Final'}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-[9px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-[0.2em]">DNI / NIT</span>
                                            <span className="text-sm font-black text-gray-900 dark:text-white font-mono">{sale.client?.dni || 'SN-0000'}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    ...
                                    <div className="p-6 bg-gray-50 dark:bg-zinc-900 rounded-[2rem] border border-gray-200 dark:border-white/5 space-y-4">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-sm font-black text-gray-900 dark:text-white italic">{new Date(sale.date || '').toLocaleDateString('es-CO')}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="rounded-[2rem] border border-gray-200 dark:border-white/5 overflow-hidden">
                                    <Table isCompact removeWrapper aria-label="Detalle de productos" classNames={{ th: "bg-gray-50 dark:bg-zinc-900 text-[9px] font-black uppercase tracking-widest text-gray-400 dark:text-zinc-500 h-10 px-6", td: "py-5 border-b border-gray-100 dark:border-white/5 bg-white dark:bg-zinc-950 px-6" }}>
                                        <TableHeader>
                                                ...
                                                <TableRow key={idx}>
                                                    <TableCell>
                                                        <div className="flex flex-col">
                                                            <span className="font-black text-xs text-gray-900 dark:text-white uppercase italic tracking-tighter">{detail.product?.productName || 'Producto'}</span>
                                                        </div>
                                                    </TableCell>
                            ...
                            <div className="flex justify-end">
                                <div className="w-80 space-y-4">
                                    <div className="flex justify-between items-center px-4">
                                        ...
                                        <span className="text-sm font-black text-gray-900 dark:text-white tabular-nums">${(sale.total || 0).toLocaleString()}</span>
                                    </div>
                                    <Divider className="dark:bg-white/5" />
                                    <div className="flex justify-between items-center p-8 bg-emerald-600 dark:bg-emerald-500/20 rounded-[2rem] border border-emerald-500/20 shadow-xl shadow-emerald-500/10">
                                        <span className="text-[10px] font-black text-emerald-100 dark:text-emerald-500 uppercase tracking-[0.3em]">Total Neto</span>
                                        <span className="text-3xl font-black text-white tabular-nums tracking-tighter">${(sale.total || 0).toLocaleString()}</span>
                                    </div>
                                </div>
                            </div>
                        </ModalBody>
                        <ModalFooter className="p-8 border-t border-gray-100 dark:border-white/5 flex gap-4 rounded-b-[2rem]">
                            <Button 
                                variant="flat" 
                                className="flex-1 h-14 rounded-xl font-black uppercase text-[10px] bg-gray-50 dark:bg-zinc-900 text-gray-400 dark:text-zinc-500 hover:bg-gray-100 italic tracking-widest" 
                                onPress={onClose}
                            >
