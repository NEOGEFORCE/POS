...
func main() {
	...
	err = db.Table("cashier_closures").Where("id = ?", 94).First(&closure).Error
	if err != nil {
		log.Fatalf("Error finding closure 94: %v", err)
	}

	fmt.Printf("Closure 94 DB: ExpectedCash=%.2f, PhysicalCash=%.2f, Diff=%.2f, TotalSales=%.2f, TotalCash=%.2f\n",
